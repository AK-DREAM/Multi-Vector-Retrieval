...
# llm = LLM(model="/home/icml01/Models/llava")
...
def parse_args():
    ...
    args = parser.parse_args(args=['--dataset_name', 'manyqa',
                                   '--data_path', '/home/icml01/images/hongkong/',
...
full_data_path = args.data_path    #os.path.join(args.data_path, args.dataset_name)
...
if args.img_concept:
    ...
    # if args.is_img_retrieval:
# print(f"bbox:  {bboxes_ls}")
# print(patch_emb_by_img_ls[0].shape, patch_emb_by_img_ls[1].shape)
...
def get_eval_answer_llava(query, sub_query_str_l, patch_emb_by_img_ls = patch_emb_by_img_ls):
    ...
    if args.is_img_retrieval:
        if args.query_concept:
            full_sub_queries_ls = sub_queries_ls # [sub_queries_ls[idx] + [[queries[idx]]] for idx in range(len(sub_queries_ls))] 
    ...
    img_folder = '/home/icml01/images/hongkong/'
...
q = ['Victoria Hong Kong has many what type of buildings?']

# sq=[
# [['Hong Kong', 'Victoria Hong Kong', 'buildings']]
# ]
sq=[
[['Hong Kong', 'Victoria Hong Kong', 'buildings']]
]

print(get_eval_answer_llava(q, sq))