...
model_path = "/home/icml01/Models/LLaVA/llava-v1.5-7b"
# model_path = "/home/icml01/Models/LLaVA/llava-v1.5-7b-sub-in1-e1"
...
def parse_args():
    ...
    args = parser.parse_args(args=['--dataset_name', 'multiqa',
                                   '--data_path', '/home/icml01/datasets/',
...
def get_eval_answer_llava(query, sub_query_str_l,  patch_emb_by_img_ls):
    ...
    for idx in range(len(query)):
        ...
        tmp_ls = sub_query_str_l[idx][0].replace("\n", "").split("|")
        # tmp_ls = [[item.strip() for item in tmp_ls]]
        # tmp_ls = [[item.strip() for item in tmp_ls if item.strip() in query[idx]]]
        tmp_ls = [list(set([item.strip() for item in tmp_ls if item.strip() and item.strip() in query[idx]]))]
        if len(tmp_ls[0]) == 0:
            tmp_ls = [[query[idx]]]
    ...
    img_folder = '/home/icml01/datasets/multiqa/'
    ...
    for idx in tqdm(range(len(queries))):
        ...
        uri = '/home/icml01/datasets/multiqa/' + img
        # display(Image(filename=uri, width=300))
        prompt = system + queries[idx]
        image_f.append(uri)

        ans.append(infer_once(prompt, image_f))
    return ans



def op_prompt_llava(instruction, query, sub_query_str_l,  patch_emb_by_img_ls):
    ...
    for idx in tqdm(range(len(queries))):
        ...
        system = instruction + "You can only answer one word."
        uri = '/home/icml01/datasets/multiqa/' + img
        # display(Image(filename=uri, width=300))
        prompt = system + queries[idx]
        image_f.append(uri)
...
def eval_acc(query, sub_query_str_l,  patch_emb_by_img_ls):
    # sub_query_str_l双层列表 [	['man| wearing| on his head| in this poster| for The Blacklist (TV series)'], []]
    queries = []  
    sub_queries_ls = [] 
    grouped_sub_q_ids_ls = None
    ...
    if args.retrieval_method == "ours":
        if not args.img_concept:
            ...
            if not args.search_by_cluster:
                ...
                # grouped_sub_q_ids_ls, bboxes_overlap_ls, dependency_topk, device, prob_agg, is_img_retrieval
                try:
                    results = retrieve_by_embeddings(
                        perc_method, full_sub_queries_ls, queries, retriever, patch_emb_by_img_ls, 
                        text_emb_ls, qrels, query_count=args.query_count, parallel=args.parallel, 
                        use_clustering=args.search_by_cluster, bboxes_ls=bboxes_ls, img_file_name_ls=img_file_name_ls, 
                        bboxes_overlap_ls=bboxes_overlap_ls, clustering_topk=args.clustering_topk, 
                        grouped_sub_q_ids_ls=grouped_sub_q_ids_ls, doc_retrieval=retrieval_method, 
                        prob_agg=args.prob_agg, dependency_topk=args.dependency_topk, device=device, 
                        is_img_retrieval=args.is_img_retrieval, method=args.algebra_method, 
                        index_method=args.index_method, _nprobe_query=args.nprobe_query, 
                        dataset_name=args.dataset_name
                    )
                except Exception as e:
                    print(f"Error occurred while retrieving embeddings: {str(e)}")
    ...
    for idx in tqdm(range(len(queries))):
        ...
        # 'You are a helpful assistant. Please answer the question according to the question and picture.'
        
        system = 'Provide a clear, accurate, and concise answer that directly addresses the question. You can only answer one word.'
        uri = '/home/icml01/datasets/multiqa/' + img
        # display(Image(filename=uri, width=300))
        prompt = system + queries[idx]
        image_f.append(uri)
