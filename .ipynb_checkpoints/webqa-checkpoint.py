...
def parse_args():
    ...
    args = parser.parse_args(args=['--dataset_name', 'webqa',
...
def llava_vllm(img_ls, queries, dataset_name):  # 'You are a helpful assistant. Please answer the question according to the question and picture. You can only answer one word. '
    ...
    for idx in range(len(img_ls)):
        image = PIL.Image.open(f'/home/icml01/images/{dataset_name}/' + img_ls[idx])
        query = queries[idx]
        prompt.append({
                    "prompt": f"USER: <image>\n{system + query}\nASSISTANT:",
...
def get_eval_answer_llava(query, sub_query_str_l, true_img_ls, patch_emb_by_img_ls = patch_emb_by_img_ls):
    ...
    img_folder = '/home/icml01/images/webqa/'
...
def colbert_score(query, re_img_ls, true_img_ls, dataset_name):

    assert len(query) == len(re_img_ls) == len(true_img_ls)
    img_folder = f'/home/icml01/images/{dataset_name}/' 
    
    print(f"hit@20 :{hit_score(re_img_ls, true_img_ls, 20)}")
    print(f"hit@100 :{hit_score(re_img_ls, true_img_ls, 100)}")

    re_imgs = [x[0] for x in re_img_ls]
    
        
    return llava_vllm(re_imgs, queries)
