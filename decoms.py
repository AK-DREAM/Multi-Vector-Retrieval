...
sys.path.append("/root/autodl-tmp/RAG/opro-para/opro/prompt_utils.py")
import psutil
...
def get_eval_answer_llava(query, sub_query_str_l,  patch_emb_by_img_ls, vlm):
    ...
    for idx in tqdm(range(len(queries))):
        ...
        pilimage = PIL.Image.open(uri)

        temp_dict = {
            "prompt": f"USER: <image>\n{prompt}\nASSISTANT:",
            "multi_modal_data": {"image": pilimage},
        }
        messages.append(temp_dict)

        
    outputs = vlm.generate(messages)
    
    ans = []
    for o in outputs:
        generated_text = o.outputs[0].text.strip().lower()
