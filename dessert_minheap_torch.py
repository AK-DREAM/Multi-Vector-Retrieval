...
class DocRetrieval:
    ...
    def query_multi_queries(self, document_embs_ls, query_embedding_ls, top_k: int, num_to_rerank: int, prob_agg="prod", dataset_name="", **kwargs):
        ...
        # all_cos_scores_tensor = torch.zeros(query_count, len(query_embedding_ls[0]), len(corpus_ids)) # .to(device)
        expected_idx_ls = []
        ...
        for idx in tqdm(range(query_count)): #, desc="Querying":
            ...
            query_cos_scores_tensor = query_cos_scores_tensor/torch.sum(query_cos_scores_tensor, dim=-1, keepdim=True)
            query_cos_scores_tensor = torch.mean(query_cos_scores_tensor, dim=0).unsqueeze(0)
        
            cos_scores_top_k_values_raw, cos_scores_top_k_idx_raw = torch.topk(query_cos_scores_tensor, min(500+1, len(query_cos_scores_tensor[0])), dim=1, largest=True)
            cos_scores_top_k_values.append(cos_scores_top_k_values_raw)
            cos_scores_top_k_idx.append(cos_scores_top_k_idx_raw)
            # sample_ids = self.query(document_embs_ls, torch.cat(query_embedding_ls[idx], dim=0), top_k, num_to_rerank, prob_agg=prob_agg, query_idx=idx, **kwargs).to(device)
            # cos_scores1, cos_scores2 = singlr_query_batch_docu_cossim(sample_ids, document_embs_ls, query_embedding_ls[idx], device)
            # all_cos_scores_tensor[idx, 0, sample_ids] = cos_scores1.cpu()   # .to(device)
            # all_cos_scores_tensor[idx, 1, sample_ids] = cos_scores2.cpu()   # .cpu() .to(device)

            # del cos_scores1, cos_scores2, sample_ids
           

        cos_scores_top_k_values = torch.cat(cos_scores_top_k_values, dim=0).cpu().tolist()
        cos_scores_top_k_idx = torch.cat(cos_scores_top_k_idx, dim=0).cpu().tolist()
        ...
        # print(cos_scores_top_k_values.shape, cos_scores_top_k_idx.shape)
        # cos_scores_top_k_values = cos_scores_top_k_values.cpu().tolist()  # .to(device).tolist()  # .cpu().tolist()
        # cos_scores_top_k_idx = cos_scores_top_k_idx.cpu().tolist()  # .to(device).tolist()  # .cpu().tolist()
