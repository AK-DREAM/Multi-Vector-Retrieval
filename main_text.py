...
def parse_args():
    ...
    parser.add_argument('--clustering_number', type=float, default=0.02, help='config file')
...
def calc_sim_score(query_emb, subquery_embs, doc_embs, subdoc_embs, subdoc_cnt):
...
def my_decompose_query(full_query, dataset_name):
    if dataset_name in ['crepe', 'mscoco']:
...
def get_subquery_embs(curr_query, args, sub_queries_ls, text_model, device):
    ...
    emb = embed_text_queries_ls(text_model, sub_queries)
    nvtx.range_pop()
    return emb
...
def search_and_load_data(
    ...
    nvtx.range_push("search indexes")
...
async def work_pipeline(curr_query, sub_queries_ls, text_model, fullquery_emb, retrieval_method, all_doc_embs, all_subdoc_embs, args, device):
    task1 = asyncio.to_thread(
        get_subquery_embs, curr_query, args, sub_queries_ls, text_model, device
    )
...
if __name__ == "__main__":       
    ...
    if args.dataset_name == "crepe":
        ...
        sub_queries_ls, idx_to_rid = decompose_queries_into_sub_queries(queries, data_path, query_key_ls=query_key_ls, dataset_name="trec-covid")
        ...
        sub_queries_ls = [sub_queries_ls[str(i+1)][0] for i in range(len(sub_queries_ls))]
    ...
    query_embs = embed_text_queries_ls(text_model, queries)
    ...
    for idx in tqdm(range(query_count), desc="processing queries"):
        ...
        if not args.parallel:
            ...
            subquery_embs = get_subquery_embs(curr_query, args, sub_queries_ls, text_model, device)
            ...
            subquery_embs, sample_ids_gpu, sample_doc_embs, sample_subdoc_cnt, sample_subdoc_embs = asyncio.run(work_pipeline(
                curr_query, sub_queries_ls, text_model, 
        ...
        # reranking
        sim_score = calc_sim_score(fullquery_emb, subquery_embs, sample_doc_embs, sample_subdoc_embs, sample_subdoc_cnt)
    ...
    # 写入日志文件
    print(f"Patch count {args.patch_count}: Task completed in {elapsed:.2f} seconds")
