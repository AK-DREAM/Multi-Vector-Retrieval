...
def parse_args():
    ...
    args = parser.parse_args(args=['--dataset_name', 'strategyqa',
                                   '--data_path', 'data/',
                                   '--query_concept', '--algebra_method', 'one',
                                   '--search_by_cluster',
...
if args.is_img_retrieval:
    ...
    if (args.use_raptor):
        ...
        raptor_model = None
    samples_hash,(img_emb, img_sparse_index), patch_emb_ls, bboxes_ls = convert_samples_to_concepts_txt(args, text_model, corpus_dict, device, raptor_model=raptor_model, patch_count_ls=patch_count_ls)
...
def get_ir_result(questions, sub_queries_ls,  img_emb= img_emb, patch_emb_by_img_ls = patch_emb_by_img_ls):
    ...
    corpus, qrels = subset_corpus(corpus_dict, qrels, args.total_count)
    ...
    i = int(list(results[str(0 + 1)].keys())[0])
    return corpus_dict[str(i-1)]['text']
        

questions = ['Will the Albany in Georgia reach a hundred thousand occupants before the one in New York?']
sub_query_str_l = [
    [['Albany' , 'Georgia', 'New York', 'a hundred thousand occupants']]
]

print(get_ir_result(questions, sub_query_str_l))